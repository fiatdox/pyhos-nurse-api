...
| POST | `/api/v1/nursing-records/admit` | Upsert — ตรวจสอบจาก `an` ถ้ามีข้อมูลแล้ว → UPDATE อัตโนมัติ |
...
| POST | `/api/v1/nursing-records/vital` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/nursing` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/careplan` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/io` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/mar/admin` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/special` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/education` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/handover` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/discharge` | Upsert — ตรวจสอบจาก `an` ถ้ามีข้อมูลแล้ว → UPDATE อัตโนมัติ |
...
| POST | `/api/v1/nursing-records/fall-risk` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/braden` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/pain` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/wound-care` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/restraint` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/gcs` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| POST | `/api/v1/nursing-records/mental-health` | Upsert — ส่ง `id` ใน body เพื่อแก้ไข, ไม่ส่ง `id` เพื่อสร้างใหม่ |
...
| โมดูล | กลไก Upsert |
|-------|-------------|
| **Admit, Discharge** | ตรวจสอบจาก `an` — ถ้ามี record อยู่แล้วจะ UPDATE อัตโนมัติ ไม่ต้องส่ง `id` |
| **โมดูลที่เหลือ (2-9, 11-17)** | ส่ง `id` (number) ใน body → UPDATE / ไม่ส่ง `id` → INSERT |

### POST Request Body (ตัวอย่าง — สร้างใหม่)

ทุก POST request ต้องส่ง `an`, `ward_code`, `staff_id` เสมอ
...
### POST Request Body (ตัวอย่าง — แก้ไข)
...
| # | Module Key | Table Name | API Base Path | Endpoints | Upsert Key |
|---|-----------|------------|---------------|-----------|------------|
| 1 | admit | nursing_admit_records | /nursing-records/admit | GET, POST, | `an` |
| 2 | vital | nursing_vital_records | /nursing-records/vital | GET, POST, DELETE | `id` in body |
| 3 | nursing | nursing_progress_notes | /nursing-records/nursing | GET, POST, DELETE | `id` in body |
| 4 | careplan | nursing_care_plans | /nursing-records/careplan | GET, POST, DELETE | `id` in body |
| 5 | io | nursing_io_records | /nursing-records/io | GET, POST, DELETE | `id` in body |
| 6 | mar | nursing_medication_orders + nursing_medication_admins | /nursing-records/mar | GET×2, POST, DELETE | `id` in body |
| 7 | special | nursing_special_care_records | /nursing-records/special | GET, POST, DELETE | `id` in body |
| 8 | education | nursing_education_records | /nursing-records/education | GET, POST, DELETE | `id` in body |
| 9 | handover | nursing_handover_records | /nursing-records/handover | GET, POST, DELETE | `id` in body |
| 10 | discharge | nursing_discharge_records | /nursing-records/discharge | GET, POST | `an` |
| 11 | fall-risk | nursing_fall_risk_records | /nursing-records/fall-risk | GET, POST, DELETE | `id` in body |
| 12 | braden | nursing_braden_records | /nursing-records/braden | GET, POST, DELETE | `id` in body |
| 13 | pain | nursing_pain_records | /nursing-records/pain | GET, POST, DELETE | `id` in body |
| 14 | wound-care | nursing_wound_care_records | /nursing-records/wound-care | GET, POST, DELETE | `id` in body |
| 15 | restraint | nursing_restraint_records | /nursing-records/restraint | GET, POST, DELETE | `id` in body |
| 16 | gcs | nursing_gcs_records | /nursing-records/gcs | GET, POST, DELETE | `id` in body |
| 17 | mental-health | nursing_mental_health_records | /nursing-records/mental-health | GET, POST, DELETE | `id` in body |

**หมายเหตุ:** ทุกโมดูลใช้ `POST` เดียวสำหรับทั้งสร้างและแก้ไข (Upsert) ไม่มี PUT endpoint
